...
THR.getThreejs = function( target ) {
	...
	THR.renderer.setSize( window.innerWidth - 320, window.innerHeight );
	container.appendChild( THR.renderer.domElement );

	THR.camera = new THREE.PerspectiveCamera( 40, (window.innerWidth - 320) / window.innerHeight, 0.1, 1000 );
...
THR.onWindowResize = function() {
	...
	THR.camera.aspect = (window.innerWidth - 320) / window.innerHeight;
